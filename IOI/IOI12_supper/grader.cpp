...
int GetRequest(void) {
  ...
  if (expect_put_back) {
    printf("Not putting back color when it is not on the scaffold\n");
    exit(1);
  }
...
void PutBack(int T) {
  ...
  if (!expect_put_back) {
    printf("Putting back a color when it is already on the scaffold\n");
    exit(1);
  }

  if (!in_scaffold[T]) {
    printf("Putting back a color that is not on the scaffold\n");
    exit(1);
  }
...
void WriteAdvice(unsigned char a) {
  ...
  if (R < M) {
    ...
    printf("Advisor is providing too many bits of advice\n");
    exit(1);
  };
}
